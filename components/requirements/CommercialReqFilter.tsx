...
const CommercialReqFilter: React.FC = () => {
  ...
  const [filteredData, setFilteredData] = useState<CommercialProperty[]>([]);

  useEffect(() => {
    const fetchData = async () => {
      const propertiesCollectionRef = collection(Firebase_store, 'commercial');
      const querySnapshot = await getDocs(propertiesCollectionRef);
      const fetchedData: CommercialProperty[] = [];
      querySnapshot.forEach(doc => {
        fetchedData.push({ id: doc.id, ...doc.data() } as CommercialProperty);
      });
      setData(fetchedData);
      ...
      <View>
        <View style={styles.container}>
          <View style={styles.overlayer}>
            <Text style={styles.maintext}>Commercial Req</Text>
...
export default CommercialReqFilter;
