...
const RentalReqFilter: React.FC = () => {
  ...
  const [filteredData, setFilteredData] = useState<RentalProperty[]>([]);

  useEffect(() => {
    const fetchData = async () => {
      const propertiesCollectionRef = collection(Firebase_store, 'rental');
      const querySnapshot = await getDocs(propertiesCollectionRef);
      const fetchedData: RentalProperty[] = [];
      querySnapshot.forEach(doc => {
        fetchedData.push({ id: doc.id, ...doc.data() } as RentalProperty);
      });
      setData(fetchedData);
      ...
      <View>
        <View style={styles.container}>
          <View style={styles.overlayer}>
            <Text style={styles.maintext}>Rental Req</Text>
...
export default RentalReqFilter;
